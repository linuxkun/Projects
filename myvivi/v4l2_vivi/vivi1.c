  ...
  * 2. 把buf放入队列
  * 3. 调用buf_queue(起通知作用)通知应用进程正在请求调用
  */
 static void myvivi_buffer_queue(struct videobuf_queue *vq, struct videobuf_buffer *vb)
 {
     ...
     /* 把videobuf放入本地一个队列尾部
      * 定时器处理函数就可以从本地队列取出videobuf，添加内核链表
      */
 ...
 static int myvivi_open(struct file *file)
 {
     ...
     videobuf_queue_vmalloc_init(&myvivi_vb_vidqueue, &myvivi_video_qops,
             ...
             sizeof(struct videobuf_buffer), NULL,NULL); /* V4L2_BUF_TYPE_VIDEO_CAPTURE用于视频捕获设备,倒数第2个参数是buffer的头部大小 */
 ...
 */
 ...
 static const struct v4l2_file_operations myvivi_fops = 
 {
     ...
     .unlocked_ioctl = video_ioctl2, /* v4l2 ioctl handler，在这个函数中设置了ioctl的用法 */
 };
 ...
 static int myvivi_init(void)
 {
	 ...
	 strlcpy(v4l2_dev.name, "kun_vivi", sizeof(v4l2_dev.name));
     error = v4l2_device_register(NULL, &v4l2_dev);
	 myvivi_device->v4l2_dev = &v4l2_dev;
     /* 2.4 队列操作
